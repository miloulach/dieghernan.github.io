...
I have found some interesting things when creating this code:
  ...
  - It is possible to read the `.png` with `brick` to create the
    `RasterBrick`. The main problem with that is that for `.png` where
    one channel (Red, Green or Blue) is null tha object doesn’t plot
    well. For that reason I preferred to import it as `png` before
